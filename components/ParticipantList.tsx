...
export function ParticipantList({ participants, onAdd, onRemove, onUpdate }: ParticipantListProps) {
  return (
        ...
        {participants.map((person, index) => (
          ...
          >
            ...
            <div className="flex gap-3">
                {/* Ordered Amount */}
                <div className="relative flex-1">
                    <label className="text-[10px] uppercase text-gray-500 font-bold tracking-wider mb-1 block">Ordered</label>
                    <div className="relative">
                        ...
                        <input
                            ...
                            onChange={(e) => onUpdate(person.id, 'orderedAmount', parseFloat(e.target.value) || 0)}
                            className="block w-full pl-9 pr-2 py-2 bg-white/5 border border-white/10 rounded-lg focus:ring-1 focus:ring-blue-500 focus:outline-none transition-colors text-right text-white"
                            placeholder="0.00"
                        />
                    </div>
                </div>
                ...
                {/* Paid Amount */}
                <div className="relative flex-1">
                    <label className="text-[10px] uppercase text-gray-500 font-bold tracking-wider mb-1 block">Paid</label>
                    <div className="relative">
                        ...
                        <input
                            ...
                            onChange={(e) => onUpdate(person.id, 'paidAmount', parseFloat(e.target.value) || 0)}
                            className="block w-full pl-9 pr-2 py-2 bg-white/5 border border-white/10 rounded-lg focus:ring-1 focus:ring-green-500 focus:outline-none transition-colors text-right text-white"
                            placeholder="0.00"
                        />
                    </div>
                </div>
